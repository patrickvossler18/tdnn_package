...
arma::vec make_param_estimate(const arma::mat& X_train,
    ...
    int s_2_val = round_modified(exp( C_s_2 * log_n * (double(p)/(double(p) + 8))));

    // need to choose s_1 through tuning
    arma::vec s_1 = as<arma::vec>(tuning(X_train, Y_train, X_val, c, n_prop, C_s_2, W0));
