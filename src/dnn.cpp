...
arma::mat make_weight_mat(int n, arma::vec ord, arma::vec s_vec)
{
    ...
    for (int i = 0; i < s_vec.size(); i++)
    {
        ...
        Rcout << "make_weight_mat: s_vec[i]: " << s_vec[i];
        for (int j = 0; j < n; j++)
        {
                    ...
                    bool debug = false)
{
    ...
    if (debug)
    {
        Rcout << "de_dnn_st: making s_1 and s_2";
    }
    ...
    if (debug)
    {
        Rcout << "de_dnn_st: making weight matrices";
    }
    ...
    if (debug)
    {
        Rcout << "de_dnn_st: summing up columns";
    }
    ...
    if (debug)
    {
        Rcout << "eu_dist_mat: " << eu_dist_mat;
        Rcout << "s_1: " << s_1;
        Rcout << "s_2: " << s_2;
        Rcout << "weight_1_mat: " << weight_1_mat;
        Rcout << "weight_2_mat: " << weight_2_mat;
        Rcout << "U1:" << U1;
        Rcout << "U2:" << U2;
        Rcout << "U:" << U;
    }
                    ...
                    NumericVector W0_, bool debug = false, bool verbose = false)
{
    ...
    for (int i = 0; i < n_vals; ++i)
    {
        ...
        if (verbose)
        {
            Rcout << "tuning sequence i: " << s_seq[i];
        }
        ...
        if (verbose)
        {
            Rcout << "tuning: estimating de_dnn";
        }
                    ...
                    Nullable<NumericVector> W0_ = R_NilValue, bool debug = false, bool verbose = false)
{
    ...
    while (search_for_s)
    {
        ...
        if (verbose)
        {
            Rcout << "candidate_results: " << candidate_results;
        }
        ...
        else if (s >= tuning_mat.n_cols)
        {
            if (verbose)
            {
                Rcout << "Reaching condition that s >= tuning_mat.ncols";
                Rcout << "  s: " << s;
            }
            ...
            for (R_xlen_t i = 0; i < diff_ratio.n_rows; ++i)
            {
                ...
                if (best_s(i) == 0)
                {
                    ...
                    {
                        Rcout << "diff_ratio(i, j): " << diff_ratio(i, j);
            ...
            if (verbose)
            {
                Rcout << "Should be breaking out of the while loop since s > tuning_mat.n_cols";
                Rcout << "  s: " << s;
            }
                     ...
                     bool verbose = false)
{
    ...
    if (verbose)
    {
        Rcout << "calculating pair-wise distance matrix. This might take a while...";
    }
