...
// [[Rcpp::export]]
mat matrix_subset_logical(mat x,
                          vec y) { 
...
NumericVector dnn( NumericMatrix X, NumericVector Y, NumericMatrix X_test,
    ...
    for (int i = 0; i < X_test.nrow(); i++){
        mat single_vec(int(n),1);
        single_vec.fill(1.0);
        mat all_cols(int(p),1);
        all_cols.fill(1.0);
        ...
        mat X_test_row =  as<mat>(NumericMatrix(1, X_test.ncol(),X_test(i,_).begin()));
        // Not an issue with X_test
        // Rcout << "i: " << i << " row: "<< X_test_row << "\n";
        all_rows = single_vec * X_test_row;
        
        X_dis = as<mat>(X) - all_rows;
        ...
        mat ordered_Y(1, int(n));
        // Rcout << as<NumericVector>(Y)[order_vec];
        // vec ordered_Y;
        // ordered_Y = as<mat>(Y[order_vec]);
        mat ordered_Y_vec = as<mat>(clone(Y)).rows(as<uvec>(order_vec));
        ...
        U_vec = reshape(ordered_Y,1,int(n)) * as<mat>(weight);
        // Rcout << "Shape of Y: " << size(ordered_Y) << "\n";
        // Rcout << "Shape of weights: " << size(as<mat>(weight)) << "\n";
...
// [[Rcpp::plugins(cpp11)]]
// [[Rcpp::export]]
List de_dnn( mat X, NumericVector Y, mat X_test,
    ...
    if (W0_.isNotNull()){
        ...
        X = matrix_subset_logical(X, as<vec>(W0));
        X_test = matrix_subset_logical(X_test, as<vec>(W0));
    } 
    ...
    for (int i = 0; i < X_test.n_rows; i++){
        ...
        U_1_vec = reshape(ordered_Y,1,int(n)) * as<mat>(weight_1);
        U_2_vec = reshape(ordered_Y,1,int(n)) * as<mat>(weight_2);
        
        weight_vec = (reshape(ordered_Y, int(n), 1) % as<mat>(weight_1)).t();
        
        mat A_mat(vec{1, 1, 1, pow((1 / bc_p),(2 /   std::min(p, 3.0)) ) });
        A_mat.reshape(2,2);
        mat A_mat_inv = A_mat.i();
        
        
        mat B_mat(vec{1, 0}); 
        B_mat.reshape(2, 1);
        
        mat Coefs = A_mat_inv * B_mat;
        
        vec U_vec = Coefs(0,0) * U_1_vec + Coefs(1,0) * U_2_vec;
...
// [[Rcpp::export]]
NumericVector best_s(mat estimate_matrix){
    ...
    for(R_xlen_t i = 0; i < estimate_matrix.n_rows; ++i) {
        mat mat_row = estimate_matrix.row(i);
        mat out_diff = diff(mat_row, 1, 1);
        
        IntegerVector idx = Range(0, (estimate_matrix.n_cols)-2);
        mat out_denom = mat_row.cols(as<uvec>(idx));
        
        mat diff_ratio = diff(abs( out_diff / out_denom), 1, 1);
...
NumericVector tuning(NumericVector s_seq,  NumericMatrix X, NumericVector Y,
    ...
    mat out(n_obs, n_vals, fill::zeros);

    // loop through and get the dnn estimates for each s value in sequence
    for(uword i = 0; i < n_vals; ++i) {
        List de_dnn_estimates = de_dnn(as<mat>(X), Y, as<mat>(X_test), 
                                       s_seq[i] + 1, bc_p, W0_);
        // Change de_dnn to output a matrix even if only 1 x 1
        // vec de_dnn_preds = as<colvec>(de_dnn_estimates["estimates"]);
        vec de_dnn_preds(n_obs);
        de_dnn_preds = as<vec>(de_dnn_estimates["estimates"]);
