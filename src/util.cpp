...
arma::mat rowSums_arma(const arma::mat &x)
{
    ...
    {
        arma::rowvec row_tmp = x.row(j);
        double row_sum = arma::sum(row_tmp);
        ans(j, 0) = row_sum;
    }
    return ans;
}
...
arma::mat calc_dist_mat_cpp(const arma::mat &A, const arma::mat &B)
{
    ...
    arma::mat A_ones(1, N, arma::fill::ones);
    arma::mat B_ones(1, M, arma::fill::ones);
    arma::mat rowSums_A = rowSums_arma(A % A);
    arma::mat rowSums_B = rowSums_arma(B % B);

    arma::mat A_dots = rowSums_A * A_ones;

    arma::mat B_dots = (rowSums_B * B_ones).t();
    arma::mat last_term = 2 * (A * B.t());
    arma::mat final = A_dots + B_dots - last_term;
    return final;
}
